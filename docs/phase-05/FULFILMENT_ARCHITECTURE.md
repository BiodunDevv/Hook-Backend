...
active Market represented in the State Order. A missing primary Runner
assignment or compatible Hub creates an exception rather than an unowned task.

Runner actions advance a versioned task through acceptance, sourcing, securing,
packing, and Hub handover. A package credential is stored only as a digest and
is returned once when the Runner creates the package. Hub receiving verifies the
