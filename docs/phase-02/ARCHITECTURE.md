...
- Identity: `User` credentials plus typed customer, staff, market associate, and partner accounts.
...
- Network: Market, Dispatch Hub, Hook Partner, Market Associate Profile, and Market Associate Market Assignment.
...
- Market Associate: isolated `app/(market-associate)/market-associate` route group, preserving `/market-associate/*` URLs, with server-enforced Market Associate login and session guard.
